...
std::vector<std::function<VkResult(VkPhysicalDevice, const VkDeviceCreateInfo*, const VkAllocationCallbacks*, VkDevice*)>> allFunctions{};
std::unordered_map<VkInstance, PFN_vkCreateDevice> createDeviceFuncPtrs = {};
...
static bool vkInitialized = false;
...
{
    char msg[512];
    UNITY_LOG(s_Log, "Creating device!");

    const int additionalVkExtensionsMaxCount = 128;
    VkDeviceCreateInfo createInfoCopy = *pCreateInfo;
    auto newExtCount = createInfoCopy.enabledExtensionCount + additionalVkExtensionsMaxCount;

    auto enabledExtensions = (char**)resizeAndCopyBuffer(createInfoCopy.ppEnabledExtensionNames, createInfoCopy.enabledExtensionCount, newExtCount);
    createInfoCopy.ppEnabledExtensionNames = (const char* const*)enabledExtensions;
    enabledExtensions
        [createInfoCopy.enabledExtensionCount++] = "VK_KHR_draw_indirect_count";

    for (uint32_t i = 0u; i < createInfoCopy.enabledExtensionCount; i++) {
        sprintf(msg, "MDI: Asking VkDeviceExtension: %s\n", enabledExtensions[i]);
        UNITY_LOG(s_Log, msg);
    }

    VkPhysicalDeviceFeatures features = *createInfoCopy.pEnabledFeatures;
    features.multiDrawIndirect = true;
    createInfoCopy.pEnabledFeatures = &features;
    
    /*
    VkPhysicalDeviceMultiDrawFeaturesEXT mdi{};
    mdi.sType = VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_MULTI_DRAW_FEATURES_EXT;
    mdi.pNext = (void*)createInfoCopy.pNext;
    createInfoCopy.pNext = &mdi;
    */

    auto result = vkCreateDevice(physicalDevice, &createInfoCopy, pAllocator, pDevice);
    if (result != VK_SUCCESS) {
        sprintf(msg, "MDI: VK Can't create device %d", result);
        UNITY_LOG(s_Log, msg);
        return result;
    }
    else {
...
static VKAPI_ATTR VkResult VKAPI_CALL Hook_vkCreateInstance(const VkInstanceCreateInfo* pCreateInfo, const VkAllocationCallbacks* pAllocator, VkInstance* pInstance)
{
    ...
    if (vkCreateInstance == nullptr) {
        UNITY_LOG_ERROR(s_Log, "No create instance func");
    }

    VkInstanceCreateInfo createInfoCopy = *pCreateInfo;
    VkApplicationInfo appInfoCopy = *createInfoCopy.pApplicationInfo;
    appInfoCopy.apiVersion = VK_API_VERSION_1_2;

    createInfoCopy.pApplicationInfo = &appInfoCopy;
    VkResult result = vkCreateInstance(&createInfoCopy, pAllocator, pInstance);
    if (result != VK_SUCCESS) {
        sprintf(msg, "MDI BadVkResult: %04X\n", result);
        UNITY_LOG(s_Log, msg);

        result = vkCreateInstance(pCreateInfo, pAllocator, pInstance);
    }
    else {
        sprintf(msg, "MDI VkSuccess!\n");
        UNITY_LOG(s_Log, msg);
    }

    if (result == VK_SUCCESS) {
        createDeviceFuncPtrs[*pInstance] = (PFN_vkCreateDevice)vkGetInstanceProcAddr(*pInstance, "vkCreateDevice");
        sprintf(msg, "Got vkCreateDevice func, %p\n", createDeviceFuncPtrs.at(*pInstance));
        UNITY_LOG(s_Log, msg);
...
static VKAPI_ATTR PFN_vkVoidFunction VKAPI_CALL Hook_vkGetInstanceProcAddr(VkInstance instance, const char* funcName)
{
    ...
    if (strcmp(funcName, "vkCreateDevice") == 0) {
        /*PFN_vkCreateDevice createDeviceFuncPtr = nullptr;
        createDeviceFuncPtr = createDeviceFuncPtrs.at(instance);

        if (createDeviceFuncPtr == nullptr) {
            UNITY_LOG_ERROR(s_Log, "No vkCreateDevice for given instance\n");
            abort();
        }

        char msg[256];
        sprintf(msg, "Given out vkCreateDevice %p", createDeviceFuncPtr);
        UNITY_LOG(s_Log, msg);
        return (PFN_vkVoidFunction)createDeviceFuncPtr;*/
...
{
    UNITY_LOG(s_Log, "InterceptVulkanInitialization");
...
{
    UNITY_LOG(s_Log, "Vulkan_OnPluginLoad");
...
public:
    ...
    virtual void ExecuteMDI(void* arguments) override;
...
void RenderAPI_Vulkan::ProcessDeviceEvent(UnityGfxDeviceEventType type, IUnityInterfaces* interfaces)
{
    ...
    case kUnityGfxDeviceEventInitialize:
        ...
        UnityVulkanPluginEventConfig config_3;
        config_3.graphicsQueueAccess = kUnityVulkanGraphicsQueueAccess_DontCare;
        config_3.renderPassPrecondition = kUnityVulkanRenderPass_EnsureInside;
        config_3.flags = kUnityVulkanEventConfigFlag_SyncWorkerThreads | kUnityVulkanEventConfigFlag_EnsurePreviousFrameSubmission | kUnityVulkanEventConfigFlag_ModifiesCommandBuffersState;
        m_UnityVulkan->ConfigureEvent(3, &config_3);
...
#ifndef VK_PIPELINE_STAGE_NONE
    #define VK_PIPELINE_STAGE_NONE 0
#endif

#define ACCESS_INDIRECT_BUFFER(argName, source) UnityVulkanBuffer argName{}; if(!this->m_UnityVulkan->AccessBuffer(source, VK_PIPELINE_STAGE_NONE, VK_ACCESS_INDIRECT_COMMAND_READ_BIT, kUnityVulkanResourceAccess_ObserveOnly, &argName)){\
    UNITY_LOG_ERROR(s_Log, STRINGIFY(Failed to obtain buffer argName buffer));\
    argName.buffer = VK_NULL_HANDLE;\
    return;\
}\

void RenderAPI_Vulkan::ExecuteMDI(void* _data) {
    if (!vkInitialized)
        return;

    CustomDrawData* data = (CustomDrawData*)_data;
    if (disabled)
        return;
    ...
    if (!this->m_UnityVulkan->CommandRecordingState(&recordingState, kUnityVulkanGraphicsQueueAccess_DontCare)) {
        UNITY_LOG_WARNING(s_Log, "MDI: Unable to obtain current command buffer");
        return;
    }

    if (recordingState.commandBuffer == VK_NULL_HANDLE) {
        UNITY_LOG_ERROR(s_Log, "MDI: Current command buffer is null");
        return;
    }

    if (vkCmdDrawIndexedIndirectCountKHR == nullptr) {
        UNITY_LOG_ERROR(s_Log, "No MDI function");
        return;
    }

    // draw.
    char draw[256];
    sprintf(draw, "Drawing %p, buffer %p + %d, count %p + %d, max commands %d\n", (void*)recordingState.commandBuffer, data->buffer, data->argsOffset, data->countBuffer, data->countOffset, data->maxCommands);

    UNITY_LOG(s_Log, draw);

    ACCESS_INDIRECT_BUFFER(drawArgs, data->buffer)
    ACCESS_INDIRECT_BUFFER(countArgs, data->countBuffer)

    vkCmdDrawIndexedIndirectCountKHR(recordingState.commandBuffer, drawArgs.buffer, data->argsOffset, countArgs.buffer, data->countOffset, data->maxCommands, 20);
}
